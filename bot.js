...
client.on("messageCreate", async (message) => {  
  try {  
    ...
    // Store every message for context, even if the bot is off  
    try {  
    ...
    // 30% chance to send a meme or gif if trigger words ("meme", "funny", "gif") are present  
    ...
    // If chatting is off, do not reply further  
    if (!chatting) return;  
